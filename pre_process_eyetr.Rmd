...
In this file I aim to process all the gaze data, essentially read, clean and combines multiple files with gaze data and their metadata, then compute and aggregate gaze metrics (parsing fixation and saccades, calculating entropy and AOI summaries, pupil, etc).
see output names in the end of the code
...
#####################################################################################
# gp preprocessing function, checks for losses, averages gaze, pupil and pixelate
#####################################################################################
...
gazepath_results_stim <- readRDS("gazepath_results.rds")
gp_summary_results_stim <- readRDS("gp_summary_results.rds")
...
# tmp_rest_by_pp <- readRDS("tmp_rest_by_pp.rds")
...
# recreate the screen content column
...
plt_aoi <- ggplot()+
  ...
  geom_vline( xintercept = 1920, size = 0.8)+
  geom_hline( yintercept = 0, size = 0.8)+
  geom_hline( yintercept = 1080, size = 0.8)+
  
  geom_segment(aes(x = 448, y = 156, yend = 924), colour = "red", size = 1.5)+
  geom_segment(aes(x = 1472, y = 156, yend = 924), colour = "red", size = 1.5)+
  geom_segment(aes(x = 448, xend = 1472, y = 156), colour = "red", size = 1.5)+
  geom_segment(aes(x = 448, xend = 1472, y = 924), colour = "red", size = 1.5)+
  geom_segment(aes(x = 448, xend = 1472, y = 540), colour = "blue", size = 0.6)+
  geom_segment(aes(x = 960, y = 156, yend = 924), colour = "blue", size = 0.6)+
  
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())+
  
  xlab("screen width (mm)") +
  ylab("screen height (mm)")
  # labs(title = "Gaze entropy areas of interest")

ggsave("C:/Users/st20965/OneDrive - University of Bristol/4th Year Masters/Images for thesis/plt_aoi.png", plt_aoi, width = 10.5, height = 6.5, units = "in")
...
```{r}
dta_gp_all_fix_sacc <- readRDS("C:/Users/st20965/OneDrive - University of Bristol/4th Year Masters/2023-24 Project/Data backup/RData/dta_gp_all_fix_sacc.rds")

dta_gp_all_fix_sacc
# not we are doing this in the whole dataset bit this is actially more meaningful for stimuli screen
dta_gp_all_fix_sacc <- dta_gp_all_fix_sacc %>%
  group_by(participant, text)%>%
  # image 1024, 768
  # screen 1080, 1920
...
dta_gp_all_fix_sacc%>%
  ...
  # geom_path()+
  facet_grid(screencontent_no_na~aoi_stim_nonstim)+
  geom_vline( xintercept = 0)+
  geom_vline( xintercept = 1920)+
  geom_hline( yintercept = 0)+
  geom_hline( yintercept = 1080)+
...
plt_gaze_by_aoi <- dta_gp_all_fix_sacc%>%
  ggplot(aes(mean_x, mean_y, color = aoi_quadrant))+
...
ggsave("C:/Users/st20965/OneDrive - University of Bristol/4th Year Masters/Images for thesis/plt_gaze_by_aoi.png", plt_gaze_by_aoi, width = 10.5, height = 6.5, units = "in")



plt_aoi_regions <- ggplot()+
  # Define each AOI quadrant as a shaded rectangle
  geom_rect(aes(xmin = 448, xmax = 960, ymin = 540, ymax = 924), fill = "#1B9E77", alpha = 1) +  # Quadrant 1
  geom_rect(aes(xmin = 960, xmax = 1472, ymin = 540, ymax = 924), fill = "#D95F02", alpha = 1) +  # Quadrant 2
  geom_rect(aes(xmin = 448, xmax = 960, ymin = 156, ymax = 540), fill = "#7570B3", alpha = 1) +    # Quadrant 3
  geom_rect(aes(xmin = 960, xmax = 1472, ymin = 156, ymax = 540), fill = "#E7298A", alpha = 1)+   # Quadrant 4
  geom_rect(aes(xmin = 0, xmax = 448, ymin = 0, ymax = 1080), fill = "#66A61E", alpha = 1)+   # Quadrant 5
  geom_rect(aes(xmin = 1472, xmax = 1920, ymin = 0, ymax = 1080), fill = "#66A61E", alpha = 1)+   # Quadrant 5
  geom_rect(aes(xmin = 448, xmax = 1472, ymin = 0, ymax = 156), fill = "#66A61E", alpha = 1)+   # Quadrant 5
  geom_rect(aes(xmin = 448, xmax = 1472, ymin = 924, ymax = 1080), fill = "#66A61E", alpha = 1)+   # Quadrant 5
  
  geom_vline( xintercept = 0, size = 0.7)+
  geom_vline( xintercept = 1920, size = 0.7)+
  geom_hline( yintercept = 0, size = 0.7)+
  geom_hline( yintercept = 1080, size = 0.7)+
  
    geom_vline( xintercept = 448, colour = "red", size = 1.5)+
  geom_vline( xintercept = 1472, colour = "red", size = 1.5)+

    geom_hline( yintercept = 156, colour = "red", size = 1.5)+
  geom_hline( yintercept = 924, colour = "red", size = 1.5)+
  
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())


ggsave("C:/Users/st20965/OneDrive - University of Bristol/4th Year Masters/Images for thesis/plt_aoi_regions.png", plt_aoi_regions, width = 10.5, height = 6.5, units = "in")
...
# option 1 - grid option - devide image into a grid -lets use this based on aoi
# option 2 - kmean cluster - try this later



# note for entropy computations we technically only care about fixations 

# Define the number of possible states
num_states <- 5
all_states <- as.character(1:num_states)
max_entropy <- log2(num_states)
num_unique_states <- length(all_states)


max_transition_entropy <- log2(num_unique_states)
...
# dta_gp_all_fix_sacc$stimnopath_no_na<- if_else(dta_gp_all_fix_sacc$screencontent_no == 2,
#                                                dta_gp_all_fix_sacc$
#                                                
#                                                )

# Calculate entropy and transition entropy
# Now, let's calculate the entropies correctly

dta_gp_all_fix_only<- dta_gp_all_fix_sacc%>%
  subset(value == "f")
...
# Function to convert sequences to a single string - this is the format needed by GRPSTring
# e.g. # simple strings
# stra.vec <- c("ABCDdefABCDa", "def123DC", "A", "123aABCD", "ACD13", "AC1ABC", "3123fe")
# TransEntro(stra.vec)
fn_sequence_to_string <- function(sequence) {
  paste(sequence, collapse = "")
}

# Updated summarise step with sequence conversion
dta_gp_all_fix_only_entropy <- dta_gp_all_fix_only %>%
  group_by(participant, text) %>%
  # just double check that the rows are ordered correctly within participant x trial combinations so that the sequence is actually true to the data
  arrange(start)%>%
  summarise(
    aoi_quadrant_sequence = list(as.character(aoi_quadrant)),
    aoi_quadrant_table = list(table(factor(aoi_quadrant, levels = all_states))),
    stat_entropy = entropy::entropy(as.numeric(unlist(aoi_quadrant_table)), unit = "log2"),
    stat_entropy_maxnorm = stat_entropy / max_entropy,
    trans_entropy = GrpString::TransEntro(fn_sequence_to_string(unlist(aoi_quadrant_sequence))),
    # trans_entropy_maxnorm = trans_entropy / max_entropy,this is not quire working
    .groups = "drop"
  )


setwd("/Users/pw22812/Library/CloudStorage/OneDrive-UniversityofBristol/Research Projects/EmotionPhysio2024/2023-24 Project/Data backup/RData")
# export to the fodler above
saveRDS(dta_gp_all_fix_only_entropy, "dta_gp_all_fix_only_entropy.rds")

range(dta_gp_all_fix_only_entropy$trans_entropy_maxnorm)

# 0.000000 1.749695

# Calculate the empirical maximum transition entropy
max_empirical_transition_entropy <- max(dta_gp_all_fix_only_entropy$trans_entropy)

# Normalize transition entropy by empirical maximum
dta_gp_all_fix_only_entropy <- dta_gp_all_fix_only_entropy %>%
  mutate(trans_entropy_maxnorm = trans_entropy / max_empirical_transition_entropy)

range(dta_gp_all_fix_only_entropy$trans_entropy_maxnorm)

# If you need to join these results back to the original data:

colnames(dta_gp_all_fix_only_entropy)
# merge back to the full dataset
dta_gp_all_fix_only_w_entrp <- dta_gp_all_fix_only %>%
  left_join(dta_gp_all_fix_only_entropy[,c(1,2,5:8)], by = c("participant", "text"))


# do some quick checks
# Check the results
dta_gp_all_fix_only_w_entrp%>%
  as.tibble()%>%
  group_by(participant, screencontent_no_na, text)%>%
  summarise_if(is.numeric, mean, na.rm = T)%>%
  
  ggplot(aes(x = stat_entropy_maxnorm, colour = screencontent_no_na)) +
  # geom_histogram(binwidth = 0.05, alpha = .2) +
  geom_histogram(aes(y = ..density..),  # Overlay density on histogram
                 binwidth = 0.05, alpha = 0.5, position = "identity") +
 geom_density(alpha = 0.3) +
  labs(title = "Distribution of Normalized Statistical Entropy",
       x = "Normalized Statistical Entropy",
       y = "Density") +
  theme_minimal()
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(x = trans_entropy_maxnorm, colour = screencontent_no_na)) +
  # geom_histogram(binwidth = 0.05, alpha = .2) +
  geom_histogram(aes(y = ..density..),  # Overlay density on histogram
                 binwidth = 0.05, alpha = 0.5, position = "identity") +
 geom_density(alpha = 0.3) +
  labs(title = "Distribution of Normalized transition Entropy",
       x = "Normalized transition Entropy",
       y = "Density") +
  theme_minimal()
...
# plot entropy by screen content
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(screencontent_no_na, stat_entropy_maxnorm))+#             colour = screencontent_no_na
  geom_jitter(alpha = .1, width = .1, size = .01)+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(screencontent_no_na, trans_entropy_maxnorm))+#             colour = screencontent_no_na
  geom_jitter(alpha = .1, width = .1, size = .01)+
  stat_summary(geom = "pointrange")+
  geom_line(aes(group = participant), size = .1)+
  theme_minimal()+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(screencontent_no_na, stat_entropy))+#             colour = screencontent_no_na
  geom_jitter(alpha = .1, width = .1, size = .01)+
  stat_summary(geom = "pointrange")+
  geom_line(aes(group = participant), size = .1)+
  theme_minimal()+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(screencontent_no_na, trans_entropy))+#             colour = screencontent_no_na
  geom_jitter(alpha = .1, width = .1, size = .01)+
  stat_summary(geom = "pointrange")+
  geom_line(aes(group = participant), size = .1)+
  theme_minimal()+
...
# plot correlations with entropy
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(stat_entropy, trans_entropy))+#             colour = screencontent_no_na
  geom_point(size = .1)+
  geom_smooth(aes(group = participant), size = .1, se = F, method = 'lm')+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")+
  facet_grid(~screencontent_no_na)
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(stat_entropy, duration))+#             colour = screencontent_no_na
  geom_point(size = .1)+
  geom_smooth(aes(group = participant), size = .1, se = F, method = 'lm')+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(trans_entropy, duration))+#             colour = screencontent_no_na
  geom_point(size = .1)+
  geom_smooth(aes(group = participant), size = .1, se = F, method = 'lm')+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(trans_entropy, order))+#             colour = screencontent_no_na
  geom_point(size = .1)+
  geom_smooth(aes(group = participant), size = .1, se = F, method = 'lm')+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")+
...
View(dta_gp_all_fix_only_w_entrp)
```


the plots above show what we would expect entropy is higher in stim lower in fixations and rest period


now checkl based on paaths

```{r}
??rank
rank(dta_gp_all_fix_only_w_entrp$trans_entropy)
dta_gp_all_fix_only_w_entrp$entrp_rnk<- rank(dta_gp_all_fix_only_w_entrp$trans_entropy)


dta_gp_all_fix_only_w_entrp%>%
  subset(screencontent_no == 3 & participant == "140")%>%
  group_by(participant,trial_no,stimnopath_no_na)%>%
  summarise_if(is.numeric, mean, na.rm = T)%>%
  
  ggplot(aes(as.numeric(trial_no), trans_entropy_maxnorm))+#             colour = screencontent_no_na
  # geom_point(size = .1)+
  # geom_text(aes(label = stimnopath_no_na))+
 stat_summary(geom = "pointrange")+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(mean_x, mean_y))+#             colour = screencontent_no_na
  geom_point(size = .1)+
  geom_path()+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")+
...
# Define all possible states and max entropy for normalization
all_states <- c("1", "2", "3", "4", "5")
max_entropy <- log2(length(all_states))

# Function to convert sequences to a single string
sequence_to_string <- function(sequence) {
  paste(sequence, collapse = "")
}
...
dta_gp_all_fix_only_entropy <- dta_gp_all_fix_only %>%
  ...
  summarise(
    ...
    stat_entropy = entropy::entropy(as.numeric(unlist(aoi_quadrant_table)), unit = "log2"),
    stat_entropy_maxnorm = stat_entropy / max_entropy,
    trans_entropy = GrpString::TransEntro(sequence_to_string(unlist(aoi_quadrant_sequence))),
    trans_entropy_maxnorm = trans_entropy / max_entropy,
    .groups = "drop"
  )

# Check transition entropy for specific sequences

dta_gp_all_fix_only%>%
  subset(participant == unique(dta_gp_all_fix_only$participant)[1] & 
           text ==unique(dta_gp_all_fix_only$text)[2])
sequence_1 <- sequence_to_string(unlist(dta_gp_all_fix_only %>%
                                          filter(participant == unique(dta_gp_all_fix_only$participant)[1], 
                                                 text == unique(dta_gp_all_fix_only$text)[1]) %>%
                                          pull(aoi_quadrant)))

sequence_2 <- sequence_to_string(unlist(dta_gp_all_fix_only %>%
                                          filter(participant == unique(dta_gp_all_fix_only$participant)[1], 
                                                 text == unique(dta_gp_all_fix_only$text)[2]) %>%
                                          pull(aoi_quadrant)))

sequence_3 <- sequence_to_string(unlist(dta_gp_all_fix_only %>%
                                          filter(participant == unique(dta_gp_all_fix_only$participant)[1], 
                                                 text == unique(dta_gp_all_fix_only$text)[3]) %>%
                                          pull(aoi_quadrant)))
...
# Calculate and print transition entropy for specific sequences
transition_entropy_1 <- GrpString::TransEntro(sequence_1)
cat("Transition Entropy for sequence 1: ", transition_entropy_1, "\n")

transition_entropy_2 <- GrpString::TransEntro(sequence_2)
cat("Transition Entropy for sequence 2: ", transition_entropy_2, "\n")

transition_entropy_1 <- GrpString::TransEntro(sequence_1)
cat("Transition Entropy for sequence 1: ", transition_entropy_1, "\n")

transition_entropy_3 <- GrpString::TransEntro(sequence_3)
cat("Transition Entropy for sequence 2: ", transition_entropy_3, "\n")
...
normalise entropy by complexity of the image

 
```{r}

setwd("/Users/pw22812/Library/CloudStorage/OneDrive-UniversityofBristol/Research Projects/EmotionPhysio2024/2023-24 Project/Data backup/Eyetracking/preprocessed/ImageJ/Image statistics")

range(dta_gp_all_fix_only_w_entrp$trans_entropy)
# [1] 0.000000 4.062667

mean(dta_gp_all_fix_only_w_entrp$trans_entropy)

range(dta_img_entropy$img_entropy)


mean(dta_img_entropy$img_entropy)
...
# Calculate the maximum possible entropy for an 8-bit image
max_entropy_img <- log2(256)  # which is 8

# Normalise the image entropy by the maximum possible entropy
dta_img_entropy <- dta_img_entropy %>%
  mutate(img_entropy_norm = img_entropy / max_entropy_img)

dta_img_entropy%>%
  ggplot(aes(img_entropy_norm))+
           geom_histogram(alpha = .5)

dta_img_entropy%>%
  ggplot(aes(img_entropy))+
           geom_histogram(alpha = .5)


```

# Arrange and display the result


```{r}
# dta_img_entropy <- dta_img_entropy %>%
#   arrange(normalised_img_entropy)

dta_gp_all_fix_only_w_entrp<-  dta_gp_all_fix_only_w_entrp%>%
 left_join(dta_img_entropy)%>%
  subset(stimnopath_no_na!= "fixcross.jpg")

dta_gp_all_fix_only_w_entrp$img_entropy<- if_else(dta_gp_all_fix_only_w_entrp$screencontent_no!=3,
                                             dta_img_entropy$img_entropy[dta_img_entropy$stimnopath_no_na == "fixcross.jpg"],
                                             dta_gp_all_fix_only_w_entrp$img_entropy
                                                  )

dta_gp_all_fix_only_w_entrp$img_entropy_norm<- if_else(dta_gp_all_fix_only_w_entrp$screencontent_no!=3,
                                             dta_img_entropy$img_entropy_norm[dta_img_entropy$stimnopath_no_na == "fixcross.jpg"],
                                             dta_gp_all_fix_only_w_entrp$img_entropy_norm
                                                  )



# normalising the gaze entropy by image entropy
dta_gp_all_fix_only_w_entrp$trans_entropy_maxnorm_by_img
dta_gp_all_fix_only_w_entrp<- dta_gp_all_fix_only_w_entrp%>%
  mutate(stat_entropy_norm_by_img = stat_entropy/img_entropy,
         trans_entropy_norm_by_img = trans_entropy/img_entropy)%>%
  mutate(stat_entropy_maxnorm_norm_by_img = stat_entropy_maxnorm/img_entropy_norm,
         trans_entropy_maxnorm_norm_by_img = trans_entropy_maxnorm/img_entropy_norm)
...
# normalised by image
# stat_entropy_norm_by_img
# stat_entropy_maxnorm_norm_by_img
# dta_gp_all_fix_only_w_entrp<- dta_gp_all_fix_only_w_entrp%>%
#   mutate(stat_entropy_norm_by_img = stat_entropy/img_entropy,
#          trans_entropy_norm_by_img = trans_entropy/img_entropy)%>%
#   mutate(stat_entropy_maxnorm_norm_by_img = stat_entropy_maxnorm/img_entropy_norm,
#          trans_entropy_maxnorm_by_img = trans_entropy_maxnorm/img_entropy_norm)
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(screencontent_no_na, stat_entropy_maxnorm_norm_by_img))+#             colour = screencontent_no_na
  geom_jitter(alpha = .1, width = .1, size = .01)+
  stat_summary(geom = "pointrange")+
  geom_line(aes(group = participant), size = .1)+
  theme_minimal()+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(stat_entropy_norm_by_img, stat_entropy_maxnorm_norm_by_img))+
  geom_point()+
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(stat_entropy_maxnorm, stat_entropy_maxnorm_norm_by_img))+
  geom_point()+
  geom_smooth(aes(group = participant), method = 'lm',se = F)+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(stat_entropy, stat_entropy_norm_by_img))+
  geom_point()+
  geom_smooth(aes(group = participant), method = 'lm',se = F)+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(trans_entropy_maxnorm, trans_entropy_maxnorm_norm_by_img))+
  geom_point()+
  geom_smooth(aes(group = participant), method = 'lm',se = F)+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")
...
dta_gp_all_fix_only_w_entrp%>%
  ...
  ggplot(aes(trans_entropy, trans_entropy_norm_by_img))+
  geom_point()+
  geom_smooth(aes(group = participant), method = 'lm',se = F)+
  theme_minimal()+
    scale_color_brewer(palette = "Dark2")
...
saveRDS(dta_gp_all_fix_only_w_entrp, "dta_gp_all_fix_only_w_entrp.rds")
...
```{r}
colnames(dta_gp_all_fix_only_w_entrp)
dta_gp_all_fix_only_w_entrp_agg<- dta_gp_all_fix_only_w_entrp%>%
  group_by(participant, text)%>%
  summarise_at(c("stat_entropy", "stat_entropy_maxnorm",
                          "trans_entropy", "trans_entropy_maxnorm",
                          "img_entropy", "img_entropy_norm",
                          "stat_entropy_norm_by_img", "trans_entropy_norm_by_img",
                          "stat_entropy_maxnorm_norm_by_img", 
                          "trans_entropy_maxnorm_by_img", 
                          "trans_entropy_maxnorm_norm_by_img"), mean,na.rm = T)
  
  
unique(dta_gp_all_fix_only_w_entrp_agg$text)
   # summarise(across(contains("entropy"), mean, na.rm = TRUE))

dta_gp_all_fix_only_w_entrp_agg


saveRDS(dta_gp_all_fix_sacc,"dta_gp_all_fix_sacc.rds")

dta_gp_all_fix_sacc$track

# merge entropy data back to fixation and saccade data
dta_gp_all_fix_sacc<- left_join(dta_gp_all_fix_sacc,
          dta_gp_all_fix_only_w_entrp_agg)

# compute metrics for fixation data
colnames(dta_gp_all_fix_sacc)
range(dta_gp_all_fix_sacc$trial)
range(as.numeric(dta_gp_all_fix_sacc$trial_no))

dta_gp_all_fix_sacc$trial[dta_gp_all_fix_sacc$trial_no == 0]
dta_gp_all_fix_sacc$trial_no #use this because trial uses 1 for rest and whatever is the first stimuli

# dta_gp_all_fix_sacc$tria


colnames(dta_gp_all_fix_sacc)

dta_gp_all_fix_sacc_agg <- dta_gp_all_fix_sacc %>%
  group_by(participant, text, trial_no, stimnopath_no_na, screencontent_no, screencontent_no_na,aoi_stim_nonstim) %>%
  ...
  summarise(
    ...
    mean_stat_entropy = mean(stat_entropy, na.rm = TRUE),
    mean_stat_entropy_maxnorm = mean(stat_entropy_maxnorm, na.rm = TRUE),
    mean_trans_entropy = mean(trans_entropy, na.rm = TRUE),
    mean_trans_entropy_maxnorm = mean(trans_entropy_maxnorm, na.rm = TRUE),
    mean_img_entropy = mean(img_entropy, na.rm = TRUE),
    mean_img_entropy_norm = mean(img_entropy_norm, na.rm = TRUE),
    mean_stat_entropy_norm_by_img = mean(stat_entropy_norm_by_img, na.rm = TRUE),
    mean_trans_entropy_norm_by_img = mean(trans_entropy_norm_by_img, na.rm = TRUE),
    mean_stat_entropy_maxnorm_norm_by_img = mean(stat_entropy_maxnorm_norm_by_img, na.rm = TRUE),
    mean_trans_entropy_maxnorm_by_img = mean(trans_entropy_maxnorm_by_img, na.rm = TRUE),
    mean_trans_entropy_maxnorm_norm_by_img = mean(trans_entropy_maxnorm_norm_by_img, na.rm = TRUE)
  ) %>%
  ungroup()
    
    # this is very slow
    # across(c("stat_entropy", "stat_entropy_maxnorm",
    #          "trans_entropy", "trans_entropy_maxnorm",
    #          "img_entropy", "img_entropy_norm",
    #          "stat_entropy_norm_by_img", "trans_entropy_norm_by_img",
    #          "stat_entropy_maxnorm_norm_by_img", 
    #          "trans_entropy_maxnorm_by_img", 
    #          "trans_entropy_maxnorm_norm_by_img"), mean, na.rm = TRUE))%>%
    
    # .groups = "drop" # This explicitly drops the grouping instead of keeping it


  # Optionally, ungroup if you want to remove the grouping structur





# Convert the data to data.table for efficiency
library(data.table)



dta_gp_all_fix_sacc_agg_dt <- setDT(dta_gp_all_fix_sacc_agg)

library(reshape2)
# Pivot wider using dcast for the selected columns
colnames(dta_gp_all_fix_sacc_agg_dt)

# dcasting to wide first so that it has a value for both aoi stim and non stim for evry one and trial, so we can gather it back

head(dta_gp_all_fix_sacc_agg_dt)


dta_gp_all_fix_sacc_agg_wide<-

  dta_gp_all_fix_sacc_agg_dt%>%
  data.table::dcast(...~ aoi_stim_nonstim,
                  value.var = 
                    c("mean_dwell_dur", "mean_fix_dur", "mean_sac_dur", "sum_dwell_dur", "sum_fix_dur", 
                                "sum_sac_dur", "mean_rms", "mean_po_gsd", "mean_sac_amp", "count_fix", "count_sacc") ,mean)%>%
  group_by(participant,text,trial_no, stimnopath_no_na, screencontent_no, screencontent_no_na)%>%
  summarise_if(is.numeric,mean, na.rm = T)%>%
  arrange(participant, trial_no, screencontent_no)
...
colnames(dta_gp_all_fix_sacc_agg_wide)
# step 1 gather all column with aoi in the names
# new column that says aoi anme, aoi value
# create a nre colun that doesn have aoi in the same
# dcast again
...
colnames(dta_gp_all_fix_sacc_agg_wide)
dta_gp_all_fix_sacc_agg_wide$aoi_name2

dta_gp_all_fix_sacc_agg_wide$aoi_value

colnames(dta_gp_all_fix_sacc_agg_wide)

dta_gp_all_fix_sacc_agg_aoi_extend<- dta_gp_all_fix_sacc_agg_wide%>%
  gather(aoi_name, aoi_value,-c(1:17))%>%
  mutate(aoi_name2 = str_remove(str_replace(aoi_name, ".*(_nonstim_aoi|_stim_aoi)", "\\1"), "_"))%>%
  # mutate(aoi_name2 = str_r(aoi_name, "_nonstim_aoi|_stim_aoi"))%>%
  mutate(et_measure = str_remove(aoi_name, "_nonstim_aoi|_stim_aoi"))%>%
  
  select(-aoi_name)%>%
  data.table::dcast(...~ et_measure,
                  value.var = 
                    c("aoi_value") ,mean)%>%
  arrange(participant, trial_no, screencontent_no)
  

colnames(dta_gp_all_fix_sacc_agg_aoi_extend)



dta_gp_all_fix_sacc_agg_aoi_extend%>%
  ggplot(aes(aoi_name2, sum_fix_dur))+
  geom_point()+
  facet_grid(~screencontent_no_na)
...
dta_gp_all_fix_sacc_agg_aoi_extend<- left_join(dta_gp_all_fix_sacc_agg_aoi_extend, 
...
# rember to add zeros to NANs before proper analysis
# count_fix
# count_sacc
# mean_dwell_dur
# mean_fix_dur
...
dta_gp_all_fix_sacc_agg_aoi_extend_nonas<- dta_gp_all_fix_sacc_agg_aoi_extend%>%
  mutate(across(c(
    mean_dwell_dur,
    mean_fix_dur,
    # mean_sac_dur,
...
Final results
these are the files that we may need to merge with other behavioural and physio data

```{r}
dta_gp_all_fix_sacc_agg_aoi_extend
dta_gp_all_fix_sacc_agg_aoi_extend_nonas

# we are storing the files here
# consider naming them more explcitelly to contain tyhe name of the study
# e.g. dta_gaze_socemo.rds
# e.g. dta_gaze_env_socemo.rds
# dta_all_env_socemo.RData
setwd("/Users/pw22812/Library/CloudStorage/OneDrive-UniversityofBristol/Research Projects/EmotionPhysio2024/2023-24 Project/Data backup/RData")


# this will contain trial and participant processed gaze data (fixation, saccades, aoi, entropy)
saveRDS(dta_gp_all_fix_sacc_agg_aoi_extend_nonas, "dta_gaze_processed.rds")

# this is the snaptshop of rdata that contains all intermediate steps for gaze processing
base::save.image("~/Library/CloudStorage/OneDrive-UniversityofBristol/Research Projects/EmotionPhysio2024/2023-24 Project/Data backup/RData/dta_gaze_env.RData")

# this will contain the bif R data that we hope we can merge with all the datasets

base::save.image("~/Library/CloudStorage/OneDrive-UniversityofBristol/Research Projects/EmotionPhysio2024/2023-24 Project/Data backup/RData/social_emophys_preprocess_all.RData")
