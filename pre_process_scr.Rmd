...
setwd("/Users/pw22812/Library/CloudStorage/OneDrive-UniversityofBristol/Research Projects/EmotionPhysio2024/2023-24 Project/Data backup/Consensys-physio")


dta_physio<- read_csv("Physio/110_Session1_GSRPulse_Calibrated_PC.csv")
View(X110_Session1_GSRPulse_Calibrated_PC)
...
# Find all subdirectories named "raw" within the base directory
raw_directories <- list.files(path = base_directory, 
                              pattern = "export", 
                              full.names = TRUE, 
                              recursive = TRUE, 
                              include.dirs = TRUE)

# Initialize an empty list to store the paths of all CSV files found
csv_files_paths <- list()

# For each "raw" directory found, list all CSV files within it
...
# `csv_files_paths` now contains the full paths to all CSV files within any "raw" subfolders

tmp_files <- list.files(pattern = ".csv", recursive = TRUE,
                        full.names = TRUE)
...
# tmp_files <- list.files(pattern = "\\.csv$", recursive = TRUE, full.names = TRUE)
